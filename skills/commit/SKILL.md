...
5. **`git add .` / `git add -A` 금지.** 반드시 관련 파일만 경로를 명시해서 stage한다.
6. **비밀값·산출물 금지.** `.env*`, `*.log`, `dist/`, `build/`, `node_modules/`, `.DS_Store`, 대용량 바이너리 등은 의도적으로 추가한 게 아니면 commit하지 않는다. 발견되면 사용자에게 확인.
